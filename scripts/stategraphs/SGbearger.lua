...
local function onattackedfn(inst, data)
    if inst.components.health ~= nil and
       ...
       (not inst.sg:HasStateTag("busy") or inst.sg:HasStateTag("frozen")) then
        ...
        if not inst.components.timer:TimerExists("hited") then
        	inst.sg:GoToState("hit")
        	inst.components.timer:StartTimer("hited", 5)
    	end
    end
end
...
local function onattackfn(inst)
    if inst.components.health ~= nil and
        ...
        local ents = TheSim:FindEntities(x,y,z, range, nil,{ "FX", "INLIMBO", "notarget", "noattack", "flight", "invisible", "playerghost", "player", "abigail"})
        local num = 0
	...
	State{
		...
		timeline=
		{
			...
			TimeEvent(20*FRAMES, function(inst)
				...
				hole.components.unevenground.radius = inst.components.combat:GetHitRange() + 1
				--print(inst.components.combat:GetHitRange())
				...
				hole.persists = false
				hole:DoTaskInTime(5, function() hole:Remove() end)
			end),
		},
