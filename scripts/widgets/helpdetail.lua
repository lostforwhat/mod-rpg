...
function HelpDetail:Layout()
    self.frame = self.proot:AddChild(TEMPLATES.CurlyWindow(260, 160, .6, .6, 39, -25))
    ...
    self.close_button:SetPosition(0, -80)
...
function HelpDetail:SetContent()
    self.text = self.frame_bg:AddChild(Text(BODYTEXTFONT, 24, "", {0.1, 0.9, 0.55, 1}))
    self.text:SetPosition(-10, 0)
    self.text:SetRegionSize(240, 140)
    ...
    self.text:SetMultilineTruncatedString(str, 7, 240, 18, "...", false)
end
